...
namespace Sports.Common
{
    public class StringSecurity
    {
        ...
        {
            _mCsp = new DESCryptoServiceProvider(); //定义访问数据加密标准 (DES) 算法的加密服务提供程序 (CSP) 版本的包装对象,此类是SymmetricAlgorithm的派生类
        }
        ...
        public string EncryptString(string value)
        {
            try
            {
                ...
                ct = _mCsp.CreateEncryptor(Convert.FromBase64String(Ckey), Convert.FromBase64String(Civ));
        ...
        public string DecryptString(string Value)
        {
            try
            {
                ...
                ct = _mCsp.CreateDecryptor(Convert.FromBase64String(Ckey), Convert.FromBase64String(Civ));
        ...
        private readonly SymmetricAlgorithm _mCsp; //声明对称算法变量
